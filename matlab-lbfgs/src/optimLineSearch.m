...
%             this.lsmethod = simpleLineSearch();
        ...
        function [X, fval] = runOptimLineSearch(this, func, X0, options, para, fold, inv2Precond,RESCALE)
            this.func = func;
            this.curX = X0;
            ...
            this.feval = 0;
            this.fold = fold;
            this.data_str = para.data_str;
            this.num_fold = para.num_fold;
...
%                     tmpX = this.curX + this.curstepsize * this.curDir;
        ...
        function [] = optimdisplay(this)
            switch this.Display
                case 'iter-detailed'
                    if (this.iter == 0)
                        fprintf('Dataset: %s | Fold:%2d / %d | Iter:%6d | feval:%6d | f(x):%10.4f | step:%7.3f | optimality:%10.5f\n',this.data_str, this.fold, this.num_fold, this.iter, this.feval, this.curfval, 0, max(this.curgrad));
                    else
                        fprintf('Dataset: %s | Fold:%2d / %d | Iter:%6d | feval:%6d | f(x):%10.4f | step:%7.3f | optimality:%10.5f\n',this.data_str, this.fold, this.num_fold,this.iter, this.feval, this.curfval, this.curstepsize, max(this.curgrad));
                    end
                case 'off'    
            end
        end
